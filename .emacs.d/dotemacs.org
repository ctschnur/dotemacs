...
** python
Remarks on ~python-mode~ and installation on windows: 
- Windows doesn't come with a system python. But ~python-mode~ tries to get the system-python executable. This doesn't work on Windows, if no ~python.exe~ is in the PATH, which results in an error and needing two attempts to actually open the ~.py~ file. One way to fix this is to install a pure [[https://www.python.org/downloads/windows/][python]] and add the link to PATH. On top of this standard python, I also usually install anaconda, for data-processing and virtual environment needs. 
