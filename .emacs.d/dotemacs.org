...
#+BEGIN_SRC emacs-lisp
  ...
  (scroll-bar-mode 0)  ;; by default, turn off scrollbars
  (if (equal system-type 'windows-nt)
      (progn
        ;; chances are that i'm looking into code together with other people
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (add-hook 'god-mode-disabled-hook #'my-god-mode-update-modeline)
   ...
   (add-hook 'help-mode-hook (lambda () (god-local-mode-pause)))
...
#+RESULTS:
| helm-gtags-mode | flycheck-mode | lsp |
...
| cs-init-pyvenv | dumb-jump-mode | (lambda nil (require 'lsp-python-ms) (lsp)) | lsp-mode | (lambda nil (interactive) (define-key python-mode-map (kbd C-)) 'sp-forward-slurp-sexp) (define-key python-mode-map (kbd C-}) 'sp-forward-barf-sexp) (define-key python-mode-map (kbd <M-up>) 'sp-splice-sexp-killing-backward)) | (lambda nil (define-key python-mode-map (kbd C-, b) 'printbreakpoint)) |
**** lsp-treemacs
- Installation: ~use-package lsp-treemacs~ didn't work, so I installed it manually
#+BEGIN_SRC emacs-lisp
...
#+RESULTS:
**** dap-mode
something like lsp-mode, but for debugging purposes
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package flycheck
    ...
    (add-hook 'c-mode-hook 'flycheck-mode)
    (add-hook 'c++-mode-hook 'flycheck-mode)
    (define-key c++-mode-map (kbd "C-, C-e") 'list-flycheck-errors)
    (define-key c++-mode-map (kbd "C-, C-n") 'flycheck-next-error)
    (define-key c++-mode-map (kbd "C-, C-p") 'flycheck-previous-error))
#+END_SRC
...
** lisp
Install lisp according to the instructions [[https://lisp-lang.org/learn/getting-started/][here]]. 
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package browse-at-remote
    :ensure t
    :config (global-set-key (kbd "C-, m b")
                            'browse-at-remote))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (when (display-graphic-p)
    ...
    (cs-frame-resize-make-frame-almost-fit-desktop))
#+END_SRC
