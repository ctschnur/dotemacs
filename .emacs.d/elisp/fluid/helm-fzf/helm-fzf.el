...
(defvar cs-this-script-absolute-dir (file-name-directory load-file-name))

;; TODO: figure out how to call my custom fzf function. So far it didn't work
...
(defun helm-fzf--do-candidate-process ()
  (let* ((cmd-args (-filter 'identity ;; (list helm-fzf-executable
                                      ...
                                      ;;       "-f"
                            ;;       helm-pattern)
                            (list (concat cs-this-script-absolute-dir "run-myfd.sh")
                                  "~"
                                  helm-pattern)
                            ))
